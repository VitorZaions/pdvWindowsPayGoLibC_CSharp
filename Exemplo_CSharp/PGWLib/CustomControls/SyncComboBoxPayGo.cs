...
namespace CustomControls.SyncControls
{
    ...
    public class SyncComboBoxPayGo : UserControl
    {
        ...
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public void EnumDataSource<T>() where T : struct
        {
            cmbList.DataSource = (from T value in Enum.GetValues(typeof(T)) select new ItemData<T>(value)).ToArray();
        }

        public void EnumDataSource<T>(params T[] excluded) where T : struct
        {
            var dataSource = (from T value in Enum.GetValues(typeof(T)) where !excluded.Contains(value) select new ItemData<T>(value)).ToArray();
            cmbList.DataSource = dataSource;
        }
        ...
        {
            get { return Convert.ToString(cmbList.SelectedValue); }
